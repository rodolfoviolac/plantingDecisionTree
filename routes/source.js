...
for(i=0; i<quantidadeAmostra; i++) {
  ...
  let temperatura = Math.round(getRandomArbitrary(-8,25));
  dat.push(['Chuva', 'Chuva', temperatura, ph, luminosidade, umidade, pluviometrico])
  res.push(Math.round(getRandomArbitrary(0,35)));
}
...
for(i=0; i<quantidadeAmostra; i++) {
  ...
  let temperatura = Math.round(getRandomArbitrary(-8,29));
...
for(i=0; i<quantidadeAmostra; i++) {
  ...
  let temperatura = Math.round(getRandomArbitrary(-30,18));
  dat.push(['Nubaldo', 'Chuva', temperatura, ph, luminosidade,umidade, pluviometrico])
  res.push(Math.round(getRandomArbitrary(12,30)));
}
...
for(i=0; i<quantidadeAmostra; i++) {
  let pluviometrico = Math.round(getRandomArbitrary(0,20));
  let umidade = Math.round(getRandomArbitrary(0,15));
  let luminosidade = Math.round(getRandomArbitrary(12,24));
  let ph = Math.round(getRandomArbitrary(4,13));
  let temperatura = Math.round(getRandomArbitrary(2,38));
...
for(i=0; i<quantidadeAmostra; i++) {
  ...
  let temperatura = Math.round(getRandomArbitrary(-15,21));
  dat.push(['Chuva', 'Nublado', temperatura, ph, luminosidade,umidade, pluviometrico])
  res.push(Math.round(getRandomArbitrary(15,45)));
}
...
for(i=0; i<quantidadeAmostra; i++) {
  ...
  res.push(Math.round(getRandomArbitrary(8,34)));
}
